...
const CreateUser = () => {
  ...
  useEffect(() => {
    setBioLength(watch("userBio").length);
  }, [watch("userBio")]);
  ...
  const formSubmit = async (data) => {
    ...
    if (passValue == cPassValue) {
      try {
        ...
        sessionStorage.setItem("webData", JSON.stringify(response));
        console.log("Created");
        router.push("/explore");
      } catch (error) {
        console.error(error);
        ...
        <form onSubmit={handleSubmit(formSubmit)}>
          ...
          <div className="-mx-3 md:flex mb-6">
            <div className="md:w-full px-3">
              ...
              <textarea
                className={`appearance-none block w-full bg-transparent text-grey-darker border ${
                  bioLength > 500 ? "border-red-700" : "border-gray-lighter"
              ...
              <p className="text-grey-dark text-xs italic">
                Make it as long, but under {bioLength} / 500 words
              </p>
            </div>
          </div>
